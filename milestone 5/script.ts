...
let Share_resume_btn = document.getElementById("share-link-btn") as HTMLButtonElement;
...
function Show_Resume() {
    ...
    Share_resume_btn.disabled = true;
...
function Submit() {
    if (validateInputFields()) {
        ...
        Share_resume_btn.disabled = false;
        ...
        const username = Fullname.value.replace(/\s+/g, '-').toLowerCase(); // Generate a URL-friendly username
        const uniqueURL = `http://localhost:3000/resume/${username}`;
        //   document.getElementById("share-link").textContent = `Share your resume: ${uniqueURL}`;
          let Share_resume =  document.getElementById('share-link') as HTMLAnchorElement;
          Share_resume.href = uniqueURL;
    }
}
